...
struct AutoServeView: View {
    ...
    var body: some View {
        VStack {
            List{
                ForEach(formLayout, id: \.id) { layout_element in
                    HStack{
                        Spacer()
                        self.switchView(served: layout_element.type, index: layout_element.id)
                        Spacer()
                    }.listRowBackground(Color.clear)
                    .onAppear{
                        printr(self.formLayout.count)
                    }
                }
            }
            .listStyle(SidebarListStyle())
            .background(Color.b_or_w.ignoresSafeArea(edges: .all)).onAppear{
                self.getFormLayout()
                printr(self.formLayout)
            }
        }
        .navigationBarTitle(Text(title), displayMode: .inline)
        .sheet(isPresented: $isShowPhotoLibrary) {
            ImagePicker(selectedImage: self.$image, onComplete: {
    ...
    func getFormLayout() {
        ...
        DatabaseDelegate.performRequest(with: json, ret: returnType.form, completion: { rex in
...
//        formLayout = [AutoServeType_hash(type: .title("Title",subtitle: "This is a subtitle for the title above")), AutoServeType_hash(type: .imagePicker) ,AutoServeType_hash(type: .textInput(placeHolder: "Placeholder")), AutoServeType_hash(type: .multichoice(choices: ["One", "Two","Three","Four"])), AutoServeType_hash(type: .multichoice(choices: ["A", "B","C","D","E"])), AutoServeType_hash(type: .datePicker(start: Date(), end: Date().advanced(by: 20000))), AutoServeType_hash(type: .incrementPicker(min: 0, max: 100, interval: 5, units: "min"))]
    ...
    func switchView(served: AutoServeType, index: UUID) -> AnyView {
        ...
        case .textInput(let placeHolder):
                return AnyView(
                        ...
                        VStack{
                            Text(placeHolder).font(.footnote).foregroundColor(.main)
                            TextField("", text: self.$textInputs[index].unwrap(def: .constant("failed"))!).onAppear{
                                    self.textInputs[index] = ""
                                    self.formTitles[index] = placeHolder
                        }.multilineTextAlignment(.center)
                            Divider().frame(width: 200)
                        }.padding(30).background(Color.lightAccent).cornerRadius(20)
            ...
            case .title(let title, let subtitle):
                return AnyView(
                    VStack{
                        Text(title).bold()
                        Text(subtitle).font(.footnote).foregroundColor(Color.main)
                    }.padding(30)
                )
            case .datePicker(let start, let end):
                return AnyView(
                    ...
                    VStack{
                    DatePicker("", selection: $dateInputs[index].unwrap(def: .constant(Date()))!, in: start...end, displayedComponents: .date)
                        .onAppear{
            ...
            case .datetimePicker(let start, let end):
                return AnyView(
                    //the date picker for the appointment
                    VStack{
                    DatePicker("", selection: $dateInputs[index].unwrap(def: .constant(Date()))!, in: start...end)
                        .onAppear{
                            self.dateInputs[index] = start
            ...
            case .multichoice(let choices):
                return AnyView(
                    ...
                    Picker(selection: $multiChoiceInputs[index].unwrap(def: .constant(0))!, label: Text(""), content: {
    ...
    func submitForm(){
        ...
        if isSelectedImage {
            ...
            json = JsonFormat.setTaskWithImage(id: defaults.franchiseId()!, value: fullTask, image: imageData).format()
        }else{
            json = JsonFormat.setTask(id: defaults.franchiseId()!, value: fullTask).format()
        }
        //TODO: add loading view
        DatabaseDelegate.performRequest(with: json, ret: returnType.string, completion: {_ in 
            //TODO: dismiss current view if image is saved
...
extension Binding {
    func unwrap<Wrapped>(def: Binding<Wrapped>) -> Binding<Wrapped>? where Optional<Wrapped> == Value {
